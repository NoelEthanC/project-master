  ...
  for (const entity of entities) {
    ...
    for (const field of entity.schema) {
      ...
      if (field.type === 'object' && field.schema) {
        schemaFields[field.name] = new mongoose.Schema(generateNestedSchema(field.schema), { _id: false });
      } else if (field.type === 'object array' && field.schema) {
        schemaFields[field.name] = [new mongoose.Schema(generateNestedSchema(field.schema), { _id: true })];
      } else {
        ...
        switch (field.type) {
          ...
          case 'dropdown':
          case 'radio':
            schemaFields[field.name].enum = field.options.map((option) => option.value);
            break;
          case 'ref':
            schemaFields[field.name] = { type: mongoose.Schema.Types.ObjectId, required: field.required };
            break;
          case 'richtext':
            schemaFields[field.name] = { type: String, required: field.required };
            break;
          case 'boolean':
          case 'check':
            schemaFields[field.name] = { type: Boolean, required: field.required };
            break;
          case 'password':
            schemaFields[field.name] = { type: String, required: field.required };
            break;
